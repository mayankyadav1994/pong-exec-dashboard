...
def write_data_file(proj_key: str, payload: dict, refreshed_at: str) -> Path:
    ...
    games_js = json.dumps(payload["games"], ensure_ascii=False)
    sprints_js = json.dumps(payload["sprints"], ensure_ascii=False)
    text = (
        f"// Auto-generated by build_jira_data.py — do not edit by hand.\n"
        f"// Project: {PROJECTS[proj_key]['key']} · refreshed {refreshed_at}\n"
        f"const GAMES = {games_js};\n"
        f"const SPRINTS = {sprints_js};\n"
        f"const REFRESHED_AT = '{refreshed_at}';\n"
    )
